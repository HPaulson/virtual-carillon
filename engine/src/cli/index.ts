      ...
      JSON.stringify(
        {
          ...
          recentEvents: database.recentEvents(),
        },
        null,
        2,
      ),
...
program
  .command('devices')
  .description('Discover PipeWire and Bluetooth outputs')
  .action(async () =>
    console.log(
      JSON.stringify(
        { outputs: await discoverOutputs(), bluetooth: await bluetoothStatus() },
        null,
        2,
      ),
    ),
...
program
  .command('play')
  .argument('<asset>', 'asset id, for example test-bell')
  .option('-o, --output <output>', 'output id or name')
  .option(
    '-d, --distance <profile>',
    'near, church-grounds, quarter-mile, half-mile, one-mile, or custom',
  )
  .action(async (asset, options) => {
    try {
      const output = (await discoverOutputs()).find(
        (item) => item.id === options.output || item.name === options.output,
      );
      const result = await library.playAsset(asset, output, { distance: options.distance });
      database.addEvent({ asset, output: output?.name, status: 'played' });
      console.log(`Playing ${asset} (${result.filePath}) via ${result.command}`);
    } catch (error) {
...
program
  ...
  .action(() => {
    engine.stop();
    console.log('Stopped active audio.');
  });
program
  .command('test')
  ...
  .action(async () => {
    for (const asset of [
      ...
      const file = await library.resolveAndRender(asset);
      console.log(`${asset}: ${file}`);
    }
  });
program
  .command('shuffle-hymns')
  .aliases(['hymns', 'cycle-hymns'])
  .description('Play a shuffled cycle of hymns until stopped')
  .option('-n, --count <number>', 'number of hymns to play; runs continuously when omitted')
  .option('-p, --pause <seconds>', 'pause between hymns in seconds', '0')
  .option('-o, --output <output>', 'output id or name')
  .option(
    '-d, --distance <profile>',
    'near, church-grounds, quarter-mile, half-mile, one-mile, or custom',
...
program
  .command('import')
  .description('Import a user recording into the persistent library')
  .requiredOption('--name <name>', 'display name')
  .requiredOption('--file <path>', 'source audio path')
  .option('--id <id>', 'stable asset id')
  .action(async (options) => {
    const asset = await library.importRecording({
      name: options.name,
...
program
  .command('doctor')
  .description('Check runtime dependencies and audio capabilities')
  .action(async () => {
    const outputs = await discoverOutputs();
    const bluetooth = await bluetoothStatus();
    console.log(
      JSON.stringify(
        {
          config: displayConfig,
          runtime: { node: process.version, platform: process.platform },
          audio: { outputs, supported: outputs.length > 0 },
          bluetooth,
        },
        null,
        2,
...
program
  .command('instrument')
  .description('Report the virtual carillon range and bell registry')
  .action(() =>
    console.log(
      JSON.stringify(
        {
          range: CARILLON_RANGE,
          bells: CARILLON_BELLS.map(({ id, pitch, frequency, size, tailSeconds, loudness }) => ({
            id,
            pitch,
            frequency,
            size,
          ...
          defaultDistance: engine.defaultDistanceProfile,
        },
        null,
        2,
      ),
...
program
  .command('diagnose')
  .description('Report normalized score voices, register, and overlap diagnostics')
  .argument('<asset>', 'hymn asset id')
  .action((assetId) => {
    const asset = library.list().find((item) => item.id === assetId);
    if (!asset?.melody) throw new Error(`Asset is not a score-backed hymn: ${assetId}`);
    console.log(
      JSON.stringify(arrangeForCarillon(scoreFromMelody(asset.melody)).diagnostics, null, 2),
    );
  });
program
  .command('server')
  ...
  .action(async (options) => {
    ...
    await app.listen({
      host: config.host,
      port: options.port ? Number(options.port) : config.port,
    });
    console.log(
      `Virtual Carillon listening on http://${config.host}:${options.port ? Number(options.port) : config.port}`,
    );
  });
program
  .command('assets')
  .description('List built-in assets')
  .action(() => console.table(library.list()));
const schedule = program
  .command('schedule')
  .description('Inspect the server-owned Westminster, asset, and hymn schedule');
schedule
  .command('show')
  .description('Show the persisted simple schedule')
  .action(() => {
    ...
    console.log(
      JSON.stringify(
        stored
          ? { ...stored, config: toSimpleSchedule(config) }
          : { config: toSimpleSchedule(config), updatedAt: 'default' },
        null,
        2,
      ),
    );
...
schedule
  .command('reset')
  .description('Reset the persisted schedule to disabled defaults')
  .action(() => {
