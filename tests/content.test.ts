...
describe('authentic content model', () => {
  it('uses stable feast IDs and canonical-hour values in hymn metadata', () => {
    ...
    expect(
      BUILTIN_HYMNS.filter((hymn) => hymn.liturgicalTags.categories.includes('contemplative')).map(
        (hymn) => hymn.id,
      ),
    ).toEqual(['aeterna-christi-munera']);
    expect(
      BUILTIN_HYMNS.filter((hymn) => hymn.liturgicalTags.categories.includes('praise')).map(
        (hymn) => hymn.id,
      ),
    ).toEqual(expect.arrayContaining(['exsultet-orbis-gaudiis', 'o-fathers-of-our-ancient-faith']));
    expect(
      BUILTIN_HYMNS.filter((hymn) => hymn.liturgicalTags.categories.includes('confidence')).map(
        (hymn) => hymn.id,
      ),
    ).toEqual(expect.arrayContaining(['salve-regina', 'salve-regina-solemn']));
  });
