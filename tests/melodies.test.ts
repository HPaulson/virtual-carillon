...
describe('notation parsers', () => {
  ...
  it('expands ABC repeat sections so a hymn does not stop at the first strain', () => {
    const melody = parseAbc('X:1\nT:Repeat\nM:4/4\nL:1/4\nK:C\n|: C D | E F :|');
    expect(melody.notes.map((note) => note.pitch)).toEqual(['C4', 'D4', 'E4', 'F4', 'C4', 'D4', 'E4', 'F4']);
  });

  it('maps the complete GABC a–m staff positions and skips the clef group', () => {
    const score = parseGabc('name:Test; mode:1; %% (c4) (a) (b) (c) (d) (e) (f) (g) (h) (i) (j) (k)');
    expect(score.events.map((event) => event.pitch)).toEqual(['A3', 'B3', 'C4', 'D4', 'E4', 'F4', 'G4', 'A4', 'B4', 'C5', 'D5']);
  });

  it('parses MusicXML pitch, rest, divisions, and tempo', () => {
    const melody = parseMusicXml('<score-partwise><work><work-title>Test XML</work-title></work><part><measure><attributes><divisions>2</divisions></attributes><note><pitch><step>C</step><octave>4</octave></pitch><duration>2</duration></note><note><rest/><duration>1</duration></note></measure></part></score-partwise>');
  ...
  it('parses a single-note Standard MIDI file', () => {
    const bytes = Uint8Array.from([
      ...
      0x4d, 0x54, 0x72, 0x6b, 0, 0, 0, 12, 0, 0x90, 60, 100, 0x60, 0x80, 60, 64, 0, 0xff, 0x2f, 0,
    ]);
