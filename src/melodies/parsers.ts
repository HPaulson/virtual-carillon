...
export function parseGabc(source: string, id = 'gabc-score'): Score {
  ...
  return {
    ...
    provenance: {
      source: metadata.book ?? 'GABC source notation',
      license: 'Public-domain chant source; notation transcription credited by source page',
      notationFormat: 'gabc',
      authorship: metadata.transcriber,
      edition: metadata.book,
    },
  };
}
...
export function gabcToMelody(source: string, id: string): Melody {
  ...
  return {
    ...
    source: score.provenance?.source,
    license: score.provenance?.license,
    notationFormat: 'gabc',
    notation: source,
    mode: score.mode,
    rhythmicCharacter: 'free',
