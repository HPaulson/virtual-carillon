 ...
 */
export function arrangeForCarillon(
  score: Score,
  ...
  const melody = source.filter(
    (event) => event.voice === 'melody' || event.voice === 'chant' || !event.voice,
  );
  const melodySource = melody.length ? melody : source;
  const shift = chooseWholePieceTranspose(
    melodySource,
    options.melodyTargetLow ?? 60,
    options.melodyTargetHigh ?? 84,
  );
  ...
  if (options.addHarmony !== false) {
    if (score.rhythmicCharacter === 'free')
      addChantTexture(
        events,
        score,
        melodySource,
  ...
  events.sort(
    (left, right) =>
      left.startSeconds - right.startSeconds || pitchToMidi(left.pitch) - pitchToMidi(right.pitch),
  );
...
function addMeteredTexture(
  ...
  for (const [index, model] of models.entries()) {
    ...
    const localProfile =
      model.melodyActivity.simple && !model.cadence && !model.finalCadence
        ? {
            ...profile,
            motionBeats: Math.max(profile.motionBeats * 1.5, model.durationBeats),
            figure: false,
          }
        : profile;
    addMovingVoice(
    ...
    const useUpper =
      model.cadence || (!useInnerTwo && shouldUseMovingVoice(model, 4, localProfile));
    if (useInnerTwo)
      addMovingVoice(
        events,
        model,
        ...
        'inner-harmony-2',
        secondsPerBeat,
        velocity * profile.innerGain * 0.9,
        localProfile,
        tonicPc,
        scale,
      );
    if (useUpper)
      addMovingVoice(
        ...
        'upper-harmony',
        secondsPerBeat,
        velocity * profile.upperGain,
        localProfile,
        tonicPc,
        scale,
      );

    if (localProfile.figure && model.finalCadence)
      addAlternatingFigure(events, model, secondsPerBeat, velocity, localProfile);
    if (model.cadence)
      addCadenceTexture(events, model, melody, shift, secondsPerBeat, velocity, profile);
  }
}
  ...
  const models: ChordModel[] = changes
    ...
    .map((change) => {
      ...
      return {
        ...
        cadence:
          phraseEnds.some((beat) => Math.abs(beat - endBeat) < 0.08) ||
          endBeat >= totalBeats - 0.08,
...
function addBassFigure(
  ...
  pushEvent(
    ...
    bass,
    start,
    Math.max(0.24, duration * profile.bassSustain),
    velocity * profile.bassGain,
    'bass',
  );
  ...
  if (allowRetrike && model.durationBeats >= 4 && profile.bassSustain < 0.9) {
    const pulseBeat =
      model.startBeat + Math.min(model.durationBeats * 0.52, profile.motionBeats * 1.2);
    const pulsePitch = nearestChordPitch(
      model,
      bass + (mod(bass, 12) === model.rootPc ? 7 : -5),
      34,
      55,
    );
    pushEvent(
      events,
      pulsePitch,
      pulseBeat * secondsPerBeat,
      Math.min(0.65, model.durationBeats * 0.22) * secondsPerBeat,
      velocity * profile.bassGain * 0.62,
      'bass-restrike',
    );
  }
  ...
  if (
    next &&
    model.cadence &&
    model.durationBeats >= 2 &&
    Math.abs(next.voicing.notes[0] - bass) >= 3
  ) {
    ...
    if (approach !== bass && approach !== next.voicing.notes[0]) {
      pushEvent(
        events,
        approach,
        Math.max(start, (model.endBeat - 0.55) * secondsPerBeat),
        0.42 * secondsPerBeat,
        velocity * 0.28,
        'bass-passing',
      );
...
function addLowerHarmony(
  ...
  pushEvent(
    ...
    lower,
    start + stagger,
    Math.max(0.25, duration * profile.harmonySustain),
    velocity * profile.harmonyGain,
    'harmony',
  );
  if (
    model.cadence &&
    (model.melodyActivity.hasLongNote || model.melodyActivity.complex) &&
    model.durationBeats >= 4 &&
    profile.motionBeats <= 1
  ) {
    ...
    pushEvent(
      ...
      second,
      start + duration * 0.52,
      Math.min(duration * 0.32, 1.25 * duration),
      velocity * profile.harmonyGain * 0.42,
      'harmony-response',
    );
  }
}
...
function addMovingVoice(
  ...
  const stagger =
    model.startBeat < 0.01 && (voiceIndex === 2 || voiceIndex === 3)
      ? 0
  ...
  for (let index = 0; index < segmentCount; index++) {
    ...
    const noteDuration = Math.max(
      0.14,
      Math.min(segmentEnd - startBeat, segmentBeats * profile.noteLength),
    );
    pushEvent(
      events,
      pitch,
    ...
    if (
      ...
      model.cadence &&
      index > 0 &&
      Math.abs(pitch - previousPitch) >= 3 &&
      segmentBeats >= 0.7
    ) {
      ...
      if (passing !== previousPitch && passing !== pitch) {
        ...
        pushEvent(
          events,
          clampMidi(passing, range.minimum, range.maximum),
          passingStart * secondsPerBeat,
          Math.min(0.42, segmentBeats * 0.34) * secondsPerBeat,
          velocity * 0.34,
          'passing-tone',
        );
      }
...
function addAlternatingFigure(
  ...
  for (const [index, offset] of offsets.entries()) {
    ...
    const pitch =
      tones[index % tones.length] + (model.finalCadence && index === count - 1 ? 12 : 0);
    pushEvent(
      events,
      pitch,
      (model.startBeat + beat) * secondsPerBeat,
      Math.min(0.5, model.durationBeats * 0.12) * secondsPerBeat,
      velocity * profile.upperGain * 0.34,
      'alternating-figure',
    );
  }
}

function shouldUseMovingVoice(
  model: ChordModel,
  voiceIndex: number,
  profile: StyleProfile,
): boolean {
  ...
  if (
    profile.motionBeats > 0.8 ||
    !model.melodyActivity.complex ||
    model.melodyActivity.activity < 0.55
  )
    return false;
...
function addCadenceTexture(
  ...
  if (profile.octaveAtCadence) {
    const cadenceNotes = melody.filter(
      (note) =>
        note.startBeat + note.durationBeats > model.endBeat - Math.min(2, model.durationBeats) &&
        note.startBeat < model.endBeat + 0.01,
    );
    ...
    if (note) {
      ...
      if (doubled <= CARILLON_RANGE.highestMidi) {
        pushEvent(
          events,
          doubled,
          Math.max(model.startBeat, note.startBeat - 0.04) * secondsPerBeat,
          Math.min(note.durationBeats, model.endBeat - note.startBeat + 0.1) * secondsPerBeat,
          velocity * 0.22,
          'melody-doubling',
        );
      }
  ...
  if (profile.flourish) {
    ...
    for (const [index, interval] of sequence.entries()) {
      const pitch = nearestChordPitch(
        model,
        model.rootMidi + interval + (index >= sequence.length - 2 ? 12 : 0),
        68,
        98,
      );
      pushEvent(
        events,
        pitch,
        (start + index * step) * secondsPerBeat,
        Math.min(step * 0.62, 0.5) * secondsPerBeat,
        velocity * (profile.motionBeats <= 0.8 ? 0.28 : 0.2),
        'cadential-flourish',
      );
    }
  }
  ...
  pushEvent(
    events,
    finalOctave,
    Math.max(model.startBeat, model.endBeat - Math.min(0.18, model.durationBeats * 0.12)) *
      secondsPerBeat,
    Math.min(1.1, model.durationBeats * 0.32) * secondsPerBeat,
    velocity * (profile.flourish ? 0.2 : 0.15),
    'cadence-octave',
  );
}
 ...
 */
export function inferChordPlan(
  melody: ScoreNote[],
  score: Score,
  tonicPitch?: string,
): ChordChange[] {
  ...
  const scale = scaleForMode(score.mode);
  const phraseEnds = [
    ...new Set((score.phrases ?? []).filter((beat) => beat > 0 && beat < totalBeats + 0.08)),
  ].sort((a, b) => a - b);
  const boundaries = new Set<number>([0, totalBeats]);

  for (let beat = 0; beat < totalBeats; beat += barBeats)
    boundaries.add(roundBeat(Math.min(totalBeats, beat)));
  ...
  const ordered = [...boundaries]
    .filter((beat) => beat >= 0 && beat <= totalBeats)
    .sort((a, b) => a - b);
  ...
  for (let index = 0; index < ordered.length - 1; index++) {
    ...
    const candidates = harmonicCandidates(
      tonicPc,
      scale,
      score.mode,
      index > 0 && changes[index - 1]?.degree === 5,
    );
    const selected =
      candidates
        .map((candidate) => ({
...
function applySecondaryDominants(
  ...
  const secondary = harmonicCandidates(tonicPc, scale, mode, false).find(
    (candidate) => candidate.secondaryOf === 5,
  );
  if (!secondary) return;
  for (let index = 0; index < changes.length - 1; index++) {
    ...
    const notes = melody.filter(
      (note) =>
        note.startBeat < current.startBeat + current.durationBeats &&
        note.startBeat + note.durationBeats > current.startBeat,
    );
    const containsSecondaryThird = notes.some(
      (note) => mod(pitchToMidi(note.pitch) - secondary.rootPc, 12) === 4,
    );
...
function sanitizeSecondaryDominants(
  ...
  for (let index = 0; index < changes.length; index++) {
    ...
    const notes = melody.filter(
      (note) =>
        note.startBeat < change.startBeat + change.durationBeats &&
        note.startBeat + note.durationBeats > change.startBeat,
    );
    const hasSecondaryThird = notes.some(
      (note) => mod(pitchToMidi(note.pitch) - secondaryRoot, 12) === 4,
    );
...
function applyPhraseCadences(
  ...
  for (const phraseEnd of [...phraseEnds, totalBeats]) {
    const ending = changes.filter(
      (change) => Math.abs(change.startBeat + change.durationBeats - phraseEnd) < 0.08,
    );
    if (!ending.length) continue;
    const final = ending.at(-1)!;
    const inPhrase = changes.filter(
      (change) =>
        change.startBeat < phraseEnd &&
        change.startBeat + change.durationBeats >
          phraseEnd - Math.max(4, final.durationBeats * 2.2),
    );
    ...
    if (inPhrase.length >= 3) {
      ...
      overwriteChange(
        predominant,
        cadenceCandidate(2, 'predominant', tonicPc, scale, mode),
        tonicMidi,
      );
      ...
      overwriteChange(
        dominant,
        cadenceCandidate(4, 'predominant', tonicPc, scale, mode),
        tonicMidi,
      );
      overwriteChange(
        tonic,
...
function harmonicCandidates(
  tonicPc: number,
  scale: number[],
  mode: number | undefined,
  emphasizeDominant: boolean,
): HarmonicCandidate[] {
  ...
  if (emphasizeDominant)
    candidates.sort((left, right) => (right.degree === 5 ? 1 : 0) - (left.degree === 5 ? 1 : 0));
  return candidates;
}
...
function scoreCandidate(
  ...
  const notes = melody.filter(
    (note) => note.startBeat < endBeat && note.startBeat + note.durationBeats > startBeat,
  );
  let score = candidate.secondaryOf ? -1.5 : 0;
  for (const note of notes) {
    const overlap =
      Math.min(endBeat, note.startBeat + note.durationBeats) - Math.max(startBeat, note.startBeat);
    ...
    const chordContainsMelody = candidate.intervals.some(
      (interval) => mod(candidate.rootPc + interval - pitch, 12) === 0,
    );
    const isAnchor =
      note.startBeat <= startBeat + 0.05 ||
      note.startBeat + note.durationBeats >= endBeat - 0.08 ||
    ...
    if (note.startBeat + note.durationBeats >= endBeat - 0.08) {
      if (relative === mod(candidate.rootPc - tonicPc, 12)) score += 3.5;
        ...
        candidate.intervals
          .slice(1)
          .some((interval) => mod(candidate.rootPc + interval - pitch, 12) === 0)
      )
        score += 1.3;
    }
  }
  ...
  if (candidate.secondaryOf)
    score += notes.some((note) => mod(pitchToMidi(note.pitch) - candidate.rootPc, 12) === 4)
      ? 3
      : 0;
  return score;
}

function diatonicCandidate(
  degree: number,
  tonicPc: number,
  scale: number[],
  mode: number | undefined,
): HarmonicCandidate | undefined {
  ...
  const quality: ChordChange['quality'] =
    third === 3 && fifth === 6
      ? 'diminished'
      : third === 3
        ? 'minor'
        : third === 4
          ? 'major'
          : 'sus4';
  const functionalQuality =
    degree === 5 && (mode === undefined || mode === 7 || mode === 5) ? 'dominant' : quality;
...
function overwriteChange(
  change: ChordChange,
  candidate: HarmonicCandidate,
  tonicMidi: number,
): void {
  const replacement = candidateToChange(
    candidate,
    change.startBeat,
    change.durationBeats,
    tonicMidi,
  );
...
function chooseVoicing(
  ...
  const specs: VoiceSpec[] = [
    {
      name: 'bass',
      minimum: bassRange.minimum,
      maximum: bassRange.maximum,
      center: bassMaximum - 4,
    },
    { name: 'harmony', minimum: lowerMinimum, maximum: lowerMaximum, center: lowerMinimum + 3 },
    {
      name: 'inner-harmony-1',
      minimum: innerOneMinimum,
      maximum: innerOneMaximum,
      center: innerOneMinimum + 3,
    },
    {
      name: 'inner-harmony-2',
      minimum: innerTwoMinimum,
      maximum: innerTwoMaximum,
      center: innerTwoMinimum + 3,
    },
    {
      name: 'upper-harmony',
      minimum: upperMinimum,
      maximum: upperMaximum,
      center: upperMinimum + 3,
    },
  ];
  ...
  for (const [index, spec] of specs.entries()) {
    ...
    for (const partial of beam) {
      for (const note of candidates) {
        ...
        if (index === 0)
          cost += bassRootPreferenceCost(note, candidates, model, previous?.notes[0]);
        ...
        if (partial.notes.some((existing) => mod(existing - note, 12) === 0))
          cost += index >= 4 ? 3 : 8;
        if (last !== undefined) {
  ...
  return {
    notes:
      best?.notes ??
      specs.map((spec) => nearestChordPitch(model, spec.center, spec.minimum, spec.maximum)),
  };
}
...
function candidatePitches(
  ...
  return [...new Set(pitches)]
    .sort((left, right) => {
      const leftCost =
        Math.abs(left - (previous ?? spec.center)) + (mod(left, 12) === model.rootPc ? -1.5 : 0);
      const rightCost =
        Math.abs(right - (previous ?? spec.center)) + (mod(right, 12) === model.rootPc ? -1.5 : 0);
...
function bassRangeForContext(
  ...
  const inRangeDistance = Math.min(
    ...nearby
      ...
      .map((pitch) => Math.abs(pitch - previous)),
    Infinity,
  );
...
function chordPitchesNear(model: ChordModel, target: number, distance: number): number[] {
  const pitches: number[] = [];
  for (
    let pitch = Math.max(24, target - distance);
    pitch <= Math.min(96, target + distance);
    pitch++
  ) {
...
function bassRootPreferenceCost(
  ...
  return rootCandidate <= deepRegisterLimit &&
    rootCandidate < previous - 4 &&
    rootDistance - candidateDistance >= 3
    ? 0.75
    : 3;
}

function voicingTotalCost(
  left: PartialVoicing,
  right: PartialVoicing,
  previous: Voicing | undefined,
): number {
...
function parallelMotionCost(current: number[], previous: number[]): number {
  ...
  for (let index = 1; index < Math.min(current.length, previous.length); index++) {
    ...
    if (
      Math.sign(currentMotion) === Math.sign(lowerMotion) &&
      Math.abs(currentMotion) > 0 &&
      Math.abs(lowerMotion) > 0
    )
      cost += 1.4;
    if (
      (currentInterval === 0 || currentInterval === 7 || currentInterval === 5) &&
...
function nearestChordPitch(
  ...
  for (const pitchClass of model.chordPcs) {
    for (let pitch = minimum - 12; pitch <= maximum + 12; pitch++)
      if (mod(pitch, 12) === pitchClass) candidates.push(pitch);
  }
  const inRange = candidates.filter((pitch) => pitch >= minimum && pitch <= maximum);
  return (
    (inRange.length ? inRange : candidates).sort(
      (left, right) => Math.abs(left - target) - Math.abs(right - target),
    )[0] ?? clampMidi(Math.round(target), minimum, maximum)
  );
}
...
function addChantTexture(
  ...
  const ends = [
    ...new Set(
      (plan.cadences?.length ? plan.cadences : (score.phrases ?? [])).filter(
  ...
  for (const [index, phraseEnd] of ends.entries()) {
    ...
    if (addBass) {
      pushEvent(
        ...
        tonicMidi,
        phraseStartSeconds,
        Math.max(0.5, phraseDuration * 0.74),
        pulse * 0.8,
        'chant-drone',
      );
      if (phraseBeats > 5)
        pushEvent(
          events,
          register.lowTenor,
          (phraseStart + phraseBeats * 0.5) * secondsPerBeat,
          phraseDuration * 0.27,
          pulse * 0.5,
          'chant-pedal-motion',
        );
    ...
    pushEvent(
      ...
      register.lowTenor,
      phraseStartSeconds,
      Math.max(0.35, phraseDuration * 0.62),
      pulse * 0.52,
      'chant-fifth',
    );
    ...
    for (const [supportIndex, supportStart] of supportStarts.entries()) {
      const anchor =
        melody.find(
          (note) =>
            note.startBeat <= supportStart + 0.05 &&
            note.startBeat + note.durationBeats > supportStart,
        ) ??
      ...
      if (flowingTexture) {
        ...
        pushEvent(
          events,
          nearestPitchClass(chord.rootPc, register.harmonyRootMinimum, register.harmonyRootMaximum),
          supportSeconds,
          supportDuration * 0.76 * secondsPerBeat,
          pulse * 0.42,
          'chant-harmony-root',
        );
        pushEvent(
          events,
          nearestPitchClass(chord.thirdPc, register.harmonyMinimum, register.harmonyMaximum),
          supportSeconds + 0.04 * secondsPerBeat,
          supportDuration * 0.82 * secondsPerBeat,
          pulse * 0.38,
          'chant-harmony',
        );
        pushEvent(
          events,
          nearestPitchClass(chord.fifthPc, register.innerMinimum, register.innerMaximum),
          supportSeconds + 0.08 * secondsPerBeat,
          supportDuration * 0.68 * secondsPerBeat,
          pulse * 0.3,
          'chant-inner',
        );
        ...
        const isFinalAnchor =
          anchor && anchor.startBeat + anchor.durationBeats >= totalBeats - 0.05;
        if (isCadence && index === ends.length - 1 && isFinalAnchor) {
          ...
          if (doubled <= Math.min(CARILLON_RANGE.highestMidi, register.melodyHighest + 12)) {
            pushEvent(
              events,
              doubled,
              Math.max(supportStart, anchor.startBeat - 0.04) * secondsPerBeat,
              Math.min(anchor.durationBeats, supportDuration + 0.1) * secondsPerBeat,
              pulse * 0.34,
              'chant-octave',
            );
          }
        ...
        const modalPitch = nearestScalePitch(
          anchorPitch - (isCadence ? 7 : 4),
          tonicPc,
          scale,
          register.harmonyMinimum,
          register.supportCeiling,
        );
        pushEvent(
          events,
          modalPitch,
          supportSeconds,
          supportDuration * secondsPerBeat,
          pulse * 0.46,
          'chant-inner',
        );
      }
    ...
    pushEvent(
      events,
      cadencePitch,
      cadenceStart * secondsPerBeat,
      Math.min(1.6, phraseBeats * 0.3) * secondsPerBeat,
      pulse * 0.58,
      index === ends.length - 1 ? 'chant-final' : 'chant-cadence',
    );
...
function chantRegister(
  ...
  const foundationTarget = clampMidi(
    melodyLowest - 24,
    CARILLON_RANGE.lowestMidi,
    CARILLON_RANGE.highestMidi,
  );
  const foundationMinimum = clampMidi(
    foundationTarget - 7,
    CARILLON_RANGE.lowestMidi,
    CARILLON_RANGE.highestMidi,
  );
  const foundationMaximum = clampMidi(
    foundationTarget + 7,
    foundationMinimum,
    CARILLON_RANGE.highestMidi,
  );
  ...
  const [harmonyRootMinimum, harmonyRootMaximum] = boundedRange(
    tonicMidi + 5,
    Math.min(tonicMidi + 17, supportCeiling - 10),
  );
  const [harmonyMinimum, harmonyMaximum] = boundedRange(
    tonicMidi + 12,
    Math.min(tonicMidi + 25, supportCeiling - 5),
  );
...
function modalChord(
  ...
  scale: number[],
  degree: number,
): { rootPc: number; thirdPc: number; fifthPc: number } {
...
function addBassLine(
  ...
  const beatGroup =
    score.rhythmicCharacter === 'free'
      ? 8
      : Math.max(1, score.meter.numerator * (4 / score.meter.denominator));
  const used = new Set<number>();
  const chant = score.rhythmicCharacter === 'free';
  const register = chant
    ? chantRegister(
        melody,
        shift,
        chantTonicPc(melody, score.arrangement ?? defaultPlan(score), shift),
        score.mode,
      )
    : undefined;
  for (const source of melody) {
    ...
    const bass =
      chant && register
        ? nearestPitchClass(
            used.size % 2 === 1 ? mod(register.tonicMidi, 12) : register.tenorPc,
            register.tonicMidi - 7,
            register.tonicMidi + 7,
          )
    ...
    pushEvent(
      events,
      bass,
      source.startBeat * secondsPerBeat,
      Math.max(2, beatGroup) * secondsPerBeat,
      Math.max(0.2, source.velocity * 0.34),
      'bass',
    );
  }
}
...
function defaultPlan(score: Score): ArrangementPlan {
  if (score.rhythmicCharacter === 'free')
    return { style: 'contemplative', cadences: score.phrases };
  const style: ArrangementStyle =
    score.meter.numerator === 3
      ? 'flowing'
      : score.mode && [1, 2, 5, 6].includes(score.mode)
        ? 'solemn'
        : 'grand';
...
function functionForChange(change: ChordChange): HarmonicFunction {
  ...
  if (change.degree === 5 || change.quality === 'dominant' || change.romanNumeral?.includes('V'))
    return 'dominant';
...
function finalChordEndpoint(changes: ChordChange[], phraseEnd: number, tonicPc: number): number {
  const change = changes.find(
    (candidate) => Math.abs(candidate.startBeat + candidate.durationBeats - phraseEnd) < 0.08,
  );
  return change?.degree === 5
    ? 7
    : mod(pitchToMidi(change?.root ?? midiToPitch(tonicPc + 60)) - tonicPc, 12);
}

function melodyAnchorAt(
  melody: ScoreNote[],
  startBeat: number,
  endBeat: number,
  shift: number,
): number {
  const local = melody.filter(
    (note) => note.startBeat < endBeat && note.startBeat + note.durationBeats > startBeat,
  );
  return local.length
    ? Math.max(...local.map((note) => pitchToMidi(note.pitch) + shift))
    : Math.max(...melody.map((note) => pitchToMidi(note.pitch) + shift));
}
...
function nearestScalePitch(
  ...
  for (let pitch = minimum; pitch <= maximum; pitch++)
    if (scale.includes(mod(pitch - tonicPc, 12))) candidates.push(pitch);
  return (
    candidates.sort((left, right) => Math.abs(left - target) - Math.abs(right - target))[0] ??
    clampMidi(Math.round(target), minimum, maximum)
  );
}

function analyzeMelodyActivity(
  melody: ScoreNote[],
  startBeat: number,
  endBeat: number,
): MelodyActivity {
  ...
  for (let index = 1; index < notes.length; index++)
    pitchMotion += Math.min(
      12,
      Math.abs(pitchToMidi(notes[index].pitch) - pitchToMidi(notes[index - 1].pitch)),
    );
  const onsetCount = notes.filter(
    (note) => note.startBeat >= startBeat - 0.05 && note.startBeat < endBeat - 0.05,
  ).length;
  ...
  return {
    ...
    pitchMotion,
    activity,
    simple: onsetCount <= 2 || activity < 0.3,
    complex,
    hasLongNote,
    hasGap,
  };
...
function pushEvent(
  ...
  const pitch = midiToPitch(
    clampMidi(Math.round(midi), CARILLON_RANGE.lowestMidi, CARILLON_RANGE.highestMidi),
  );
  bellForPitch(pitch);
  events.push({
    pitch,
    ...
    velocity: clamp(velocity, 0.08, 0.85),
    voice,
  });
}

function chooseWholePieceTranspose(
  events: ScoreNote[],
  targetLow: number,
  targetHigh: number,
): number {
...
function diagnose(events: CarillonEvent[], transposeSemitones: number): ArrangementDiagnostics {
  ...
  if (lowCount / allMidi.length > 0.6)
    warnings.push(
      'More than 60% of events are below C3; arrangement may sound muddy or unusually low.',
    );
  if (Math.min(...melodyMidi) < 55)
    warnings.push('Melody is below G3; verify register before publishing.');
  let overlappingEvents = 0;
  const byStart = new Map<number, number>();
  for (const event of events)
    byStart.set(
      Math.round(event.startSeconds * 1000),
      (byStart.get(Math.round(event.startSeconds * 1000)) ?? 0) + 1,
    );
