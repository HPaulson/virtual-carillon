  ...
  for (const quarter of quarters) {
    ...
    for (const phraseName of phraseNames) {
      ...
      for (let noteIndex = 0; noteIndex < 4; noteIndex++) {
        ...
        events.push({
          offset: phraseOffset + noteIndex * WESTMINSTER_BEAT_SECONDS,
          pitch,
          preset: 'clock',
          duration: 0.5,
          kind: 'note',
          gain: noteIndex === 3 ? 1.28 : 1.15,
          voice: 'clock-chime',
        });
      }
...
function westminsterAsset(
  id: string,
  name: string,
  quarters: WestminsterQuarter[],
  hour?: number,
): AssetDefinition {
...
function sequenceDuration(events: SequenceEvent[], extra = 0.5): number {
  return Math.max(...events.map((event) => (event.offset ?? 0) + (event.duration ?? 0))) + extra;
}

function signalAsset(
  id: string,
  name: string,
  preset: string,
  frequency: number,
  ...
  return {
    ...
    events: Array.from({ length: count }, (_, index) => ({
      ...
      frequency,
      preset,
      duration: preset === 'deep' ? 7.5 : 4.2,
      kind: 'strike',
      gain: 0.88,
    })),
    duration: (count - 1) * spacing + (preset === 'deep' ? 7.5 : 4.2),
    instrument: preset === 'deep' ? 'tower-bell' : 'monastery-bell',
    tags: ['Divine Office', name.split('·').pop()?.trim() ?? 'Office'],
    description,
...
export const BUILTIN_ASSETS: AssetDefinition[] = [
  ...
  {
    id: 'large-church-bell',
    name: 'Large Church Tower Bell',
    type: 'bell',
    source: 'generated',
    contentKind: 'generated',
    preset: 'large',
  ...
  ...Array.from({ length: 12 }, (_, index) =>
    westminsterAsset(
      `westminster-hour-${index + 1}`,
      `Westminster Hour · ${index + 1} strike${index ? 's' : ''}`,
      ['Q4'],
      index + 1,
    ),
  ),
  {
    id: 'angelus',
    name: 'Traditional Angelus · Three groups of three',
    type: 'sequence',
    source: 'generated',
    contentKind: 'traditional',
    events: angelusEvents([3, 3, 3]),
    duration: sequenceDuration(angelusEvents([3, 3, 3])),
    instrument: 'tower-bell',
    tags: ['Angelus', 'Bell', 'Traditional'],
    sourceUrl: 'https://www.catholic.com/encyclopedia/angelus-bell',
    description:
  ...
  {
    id: 'angelus-rome',
    name: 'Angelus · Roman evening variant',
    type: 'sequence',
    source: 'generated',
    contentKind: 'traditional',
    events: angelusEvents([3, 4, 5, 1]),
    duration: sequenceDuration(angelusEvents([3, 4, 5, 1])),
    instrument: 'tower-bell',
...
export class AssetLibrary {
  ...
  constructor(
    ...
    userAssetDir = path.join(path.dirname(cacheDir), 'assets'),
  ) {
  ...
  list(): AssetDefinition[] {
    return [...BUILTIN_ASSETS, ...this.userAssets].map(
      (asset) =>
        Object.fromEntries(
          Object.entries(asset).filter(([key]) => key !== 'filePath'),
        ) as AssetDefinition,
    );
  }
  ...
  async resolveAndRender(assetId: string, options: RenderOptions = {}): Promise<string> {
    ...
    if (asset.type === 'hymn' && asset.melody)
      return this.engine.renderMelody(asset.id, asset.melody, 'bright', options);
    ...
    const bellOptions: BellRenderOptions = {
      preset: asset.preset,
  ...
  async playAsset(assetId: string, output?: AudioOutput, options: RenderOptions = {}) {
    return this.engine.playFile(await this.resolveAndRender(assetId, options), output);
  }
  ...
  async importRecording(input: UserRecordingInput): Promise<AssetDefinition> {
    ...
    if (!RECORDING_EXTENSIONS.has(extension))
      throw new Error(`Unsupported recording format: ${extension || 'none'}`);
    const source = await fsp.realpath(input.sourcePath);
    const id = sanitiseId(input.id ?? path.basename(source, extension));
    if (BUILTIN_ASSETS.some((asset) => asset.id === id))
      throw new Error(`Asset id is reserved by built-in content: ${id}`);
    ...
    const asset: AssetDefinition = {
      ...
      contentKind: 'custom',
      filePath: target,
      tags: input.tags ?? ['Recording'],
      license: input.license,
      attribution: input.attribution,
      sourceUrl: input.sourceUrl,
      liturgicalSeasons: input.liturgicalSeasons,
      feastTypes: input.feastTypes,
      liturgicalTags: input.liturgicalTags ? createLiturgicalTags(input.liturgicalTags) : undefined,
    };
    const existing = this.userAssets.findIndex((entry) => entry.id === id);
    if (existing >= 0) this.userAssets.splice(existing, 1, asset);
    else this.userAssets.push(asset);
  ...
  pathFor(id: string) {
    return path.join(this.cacheDir, `${id}-half-mile.wav`);
  }

  private find(assetId: string) {
    return [...BUILTIN_ASSETS, ...this.userAssets].find((entry) => entry.id === assetId);
  }

  private loadUserManifest(): AssetDefinition[] {
    try {
      const raw = JSON.parse(
        fs.readFileSync(path.join(this.userAssetDir, USER_MANIFEST), 'utf8'),
      ) as Array<Omit<AssetDefinition, 'filePath'> & { fileName: string }>;
  ...
  private async saveUserManifest() {
    await fsp.mkdir(this.userAssetDir, { recursive: true });
    await fsp.writeFile(
      path.join(this.userAssetDir, USER_MANIFEST),
      JSON.stringify(
        this.userAssets.map(({ filePath, ...asset }) => ({
          ...asset,
