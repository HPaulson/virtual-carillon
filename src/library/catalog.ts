...
export class HymnCatalog {
  ...
  private chooseScored(
    ...
    const scored = candidates.map((asset) => {
      ...
      const addMatch = (assetValues: string[], targetValues: string[], points: number) => {
        ...
        if (matches.length) {
          for (const match of matches) {
            score += points;
            breakdown.push({ label: match, score: points });
          }
        }
      };
      ...
      return { asset, score, baseScore, breakdown };
    });
    ...
    const selectedRank =
      selectedScore === undefined
        ...
        : [...scored]
            .sort(
              (left, right) =>
                right.baseScore - left.baseScore || left.asset.id.localeCompare(right.asset.id),
            )
