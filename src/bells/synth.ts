...
export function synthesizeBell(options: BellRenderOptions = {}): Float32Array {
  const registryBell = options.bellId
    ? getCarillonBell(options.bellId)
    : options.pitch
      ? bellForPitch(options.pitch)
      : undefined;
  const preset = options.preset ? resolvePreset(options.preset) : undefined;
  const bell = preset
    ? presetAsDefinition(preset, options.frequency)
    : (registryBell ?? presetAsDefinition(BELL_PRESETS.medium, options.frequency));
  const sampleRate = options.sampleRate ?? 44100;
  const distance = resolveDistance(
    options.distance ?? options.distanceProfile,
    options.reverb,
    options.customDistance,
  );
  ...
  const normalizer = Math.max(
    1,
    bell.partials.reduce((sum, partial) => sum + partial.amplitude, 0),
  );
  ...
  for (let frame = 0; frame < frames; frame++) {
    ...
    for (const [index, partial] of bell.partials.entries()) {
      ...
      modal +=
        partial.amplitude * attenuation * (firstMode + secondMode) * Math.exp(-t / partial.decay);
    }
    modal /= normalizer;
    ...
    const strikeEnvelope =
      (1 - Math.exp(-frame / Math.max(1, 0.0007 * sampleRate))) * Math.exp(-t / strikeDecay);
    const strike =
      strikeEnvelope *
      distance.attackGain *
...
export function applyDistanceModel(
  input: Float32Array,
  sampleRate: number,
  ...
  for (let tap = 0; tap < settings.reflectionDelays.length; tap++) {
    ...
    for (let index = delay; index < filtered.length; index++)
      result[index] += filtered[index - delay] * gain;
  }
  return result;
}
...
export function applyReverb(
  ...
  const distance: DistanceProfile =
    profile === 'small-room'
      ? 'church-grounds'
      : profile === 'church'
        ? 'quarter-mile'
        : profile === 'large-church'
          ? 'half-mile'
          : 'one-mile';
...
function resolveDistance(
  distance: DistanceProfile | undefined,
  reverb: ReverbProfile | undefined,
...
function presetAsDefinition(preset: BellPreset, frequency?: number): BellDefinition {
  return {
    ...
    tailSeconds:
      Math.max(preset.duration, ...preset.partials.map((partial) => partial.decay)) * 2.7 + 1,
