...
export class LiturgicalCalendarClient {
  ...
  async getYear(year: number, calendar = this.config.defaultCalendar): Promise<LiturgicalDay[]> {
    ...
    // Include the locale in the cache key so older Latin/default-locale caches
    // cannot continue supplying non-English celebration names.
    const cachePath = path.join(
      this.config.cacheDir,
      `litcal-${calendar}-${year}-${DEFAULT_LOCALE}.json`,
    );
  ...
  private urlFor(year: number, calendar: LiturgicalCalendarName): string {
    ...
    return `${(this.config.baseUrl ?? DEFAULT_BASE_URL).replace(/\/$/, '')}${pathPart}?year_type=CIVIL&locale=${DEFAULT_LOCALE}`;
  }
}
  ...
  const celebrations = (day.celebrations ?? []).map((event) => {
    const tags =
      ...
      inferLiturgicalTags({
        ...
        rank: event.rank,
        grade: event.grade,
        feastTypes: event.feastType ? [event.feastType] : [],
        common: event.common,
      });
    return {
      ...event,
...
function normaliseCelebration(
  ...
  const feastType = stringValue(value.feast_type ?? value.type);
  const common = stringArray(value.common);
  const colors = stringArray(value.color ?? value.liturgical_color);
  const localColors = stringArray(value.color_lcl);
  const tags = inferLiturgicalTags({
    ...
    rank,
    common,
    feastTypes: value.feast_type ? [feastType as string] : [],
  });
  return [
    {
      key: stringValue(value.event_key ?? value.key),
      eventIndex: numberValue(value.event_idx),
      name,
      rank,
      rankId: rankId(grade, rank),
      rankAbbreviation: stringValue(value.grade_abbr),
      grade,
      color: colors[0],
      colors,
      localColors,
      season,
      seasonLocal: stringValue(value.liturgical_season_lcl),
      feastType: stringValue(value.feast_type),
      eventType: stringValue(value.type),
      common,
      commonLocal: stringValue(value.common_lcl),
      readings: recordOfStrings(value.readings),
      liturgicalYear: stringValue(value.liturgical_year),
      psalterWeek: numberValue(value.psalter_week),
      isVigilMass: booleanValue(value.is_vigil_mass),
      isVigilFor: stringValue(value.is_vigil_for),
      hasVigilMass: booleanValue(value.has_vigil_mass),
      hasVesperI: booleanValue(value.has_vesper_i),
      hasVesperII: booleanValue(value.has_vesper_ii),
      holyDayOfObligation: booleanValue(value.holy_day_of_obligation),
      liturgicalTags: tags,
    },
  ];
}
