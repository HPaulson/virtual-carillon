...
export function inferLiturgicalTags(input: {
  ...
  if (text.includes('pastor') || text.includes('bishop') || text.includes('pope'))
    addCategory('pastors');
  if (text.includes('missionar')) addCategory('missionaries');
  if (text.includes('educator')) addCategory('educators');
  if (text.includes('works of mercy')) addCategory('works-of-mercy');
  if (text.includes('holy women')) addCategory('holy-women');
...
function isSaintCelebration(text: string): boolean {
  return /(^| )(saint|saints|st |sts |san |santa |santo |sanct|beat|blessed )/.test(text);
}
