...
export async function writeWav(filePath: string, samples: Float32Array, sampleRate = 44100): Promise<void> {
  ...
  buffer.writeUInt32LE(sampleRate, 24); buffer.writeUInt32LE(sampleRate * channels * bitsPerSample / 8, 28);
...
export function wavDuration(samples: Float32Array, sampleRate: number): number { return samples.length / 2 / sampleRate; }
