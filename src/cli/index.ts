...
import { DEFAULT_SCHEDULES } from '../scheduler/types.js';
import { Scheduler } from '../scheduler/scheduler.js';
import { LiturgicalCalendarClient } from '../liturgical/litcal.js';
import { LiturgicalResolver } from '../liturgical/resolver.js';
import { MONASTERY_PROFILE } from '../scheduler/profiles.js';
...
const liturgicalCalendar = new LiturgicalCalendarClient({ enabled: config.liturgicalCalendarEnabled, baseUrl: config.liturgicalCalendarBaseUrl, calendar: config.liturgicalCalendar, cacheDir: path.join(config.dataDir, 'litcal') });
const liturgicalResolver = new LiturgicalResolver(liturgicalCalendar, library, hymnCatalog);
const schedules = database.schedules();
if (!schedules.length) database.replaceSchedules(DEFAULT_SCHEDULES);
const scheduler = new Scheduler(database.schedules(), async (asset, output) => { await library.playAsset(asset, (await discoverOutputs()).find((item) => item.id === output || item.name === output)); }, console.log, async (entry, now) => liturgicalResolver.resolve(entry, now));

const program = new Command();
program.name('virtual-carillon').description('Virtual Carillon church bell engine').version('0.1.0');
program.command('status').description('Show engine, scheduler, audio, and Bluetooth status').action(async () => { console.log(JSON.stringify({ config, outputs: await discoverOutputs(), bluetooth: await bluetoothStatus(), schedules: scheduler.getEntries(), recentEvents: database.recentEvents() }, null, 2)); });
...
program.command('profile').description('Show or install an optional schedule profile').argument('<name>', 'profile name, currently monastery').option('--install', 'add missing entries without replacing existing schedules').action((name, options) => { if (name !== 'monastery') throw new Error(`Unknown profile: ${name}`); if (!options.install) { console.log(JSON.stringify(MONASTERY_PROFILE, null, 2)); return; } const existing = database.schedules(); const ids = new Set(existing.map((entry) => entry.id)); const merged = [...existing, ...MONASTERY_PROFILE.filter((entry) => !ids.has(entry.id))]; database.replaceSchedules(merged); scheduler.setEntries(merged); console.log(`Installed ${merged.length - existing.length} monastery schedule entries; existing schedules were preserved.`); });
program.command('doctor').description('Check runtime dependencies and audio capabilities').action(async () => { const outputs = await discoverOutputs(); const bluetooth = await bluetoothStatus(); console.log(JSON.stringify({ config, runtime: { node: process.version, platform: process.platform }, audio: { outputs, supported: outputs.length > 0 }, bluetooth }, null, 2)); });
...
program.command('server').description('Run the Home Assistant API server').option('-p, --port <port>', 'port').action(async (options) => { const app = await createServer({ engine, library, database, scheduler, liturgicalCalendar, hymnCatalog }); scheduler.start(); await app.listen({ host: config.host, port: options.port ? Number(options.port) : config.port }); console.log(`Virtual Carillon listening on http://${config.host}:${options.port ? Number(options.port) : config.port}`); });
