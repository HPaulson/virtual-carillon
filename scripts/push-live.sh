...
COPYFILE_DISABLE=1 tar \
  ...
  --exclude='**/*.pyc' \
  -czf "$LOCAL_ARCHIVE" \
  -C "$ROOT_DIR/custom_components/virtual_carillon" \
  .
...
COPYFILE_DISABLE=1 tar \
  ...
  -czf "$ENGINE_ARCHIVE" \
  -C "$ROOT_DIR" \
  Dockerfile package.json pnpm-lock.yaml tsconfig.json eslint.config.mjs .prettierrc.json scripts engine add-on README.md LICENSE
