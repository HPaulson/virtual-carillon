...
# ssh concatenates command arguments before sending them to the remote shell,
# which means empty positional arguments are otherwise lost (and a container
# name can accidentally be interpreted as the reset date). Pass both values as
# shell-quoted environment assignments instead.
printf -v REMOTE_COMMAND 'RESET_DATE_ARG=%q CARILLON_CONTAINER_ARG=%q bash -s' \
  "$RESET_DATE" "${CARILLON_CONTAINER:-}"

ssh "$REMOTE_HOST" "$REMOTE_COMMAND" <<'REMOTE_SCRIPT'
set -Eeuo pipefail

reset_date="${RESET_DATE_ARG:-}"
